...
/**
 * Self-contained pub.dev fetcher (no imports outside this file).
 * Vercel serverless fails to load `../lib/*` at runtime (FUNCTION_INVOCATION_FAILED).
 * Keep logic aligned with `lib/pubPackagesFetch.ts` (used by Vite dev middleware).
 */
...
function sanitizePubPackageList(body: unknown): string[] {
  const raw =
    typeof body === 'object' &&
    body !== null &&
    'packages' in body &&
...
async function aggregatePubPackages(body: unknown): Promise<Record<string, PubPackageRecord>> {
